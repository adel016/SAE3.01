...
class ControllerMeteotheque {
    ...
    // Affiche la Meteothèque de l'utilisateur
    public static function showMeteotheque() {
        ...
        $requetes = $meteothequeRepo->findByUserId($utilisateur->getId());
    ...
    public static function saveRequest() {
        ...
        $meteo = new Meteotheque(
            0, // ID généré automatiquement
            $utilisateurId,
            $regionName,
            "Requête sauvegardée pour la région $regionName",
            date('Y-m-d H:i:s')
        );
